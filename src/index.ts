...
import {  render as convergedRender } from "@solenopsys/converged-renderer";
...
function render(ui: Ui, options: Options = {}): Result {
  let { container, baseElement = container, queries, wrapper, location } = options;
  ...
  const routedUi: Ui =
    typeof location === "string"
      ? lazy(async () => {
          try {
            const { createMemoryHistory, MemoryRouter } = await import("@solenopsys/converged-router");
...
export function renderHook<A extends any[], R>(
  ...
  const [dispose, owner, result] = root(dispose => {
    if (
...
export function renderDirective<A extends any, U extends A, E extends HTMLElement>(
  ...
  const arg= $(options?.initialValue as U);
...
function cleanupAtContainer(ref: Ref) {
  ...
  if (typeof dispose === 'function') {
    ...
    console.warn('conveged-test: dispose is not a function - maybe your tests include multiple solid versions!');
  }
